...
// ==================== MÓDULO: VIPS CON 2 PESTAÑAS - VERSIÓN PROFESIONAL ====================
function initVips() {
  console.log("🔄 Inicializando módulo VIPs con 2 pestañas...");
  
  // ========== NAVEGACIÓN ENTRE PESTAÑAS ==========
  const vipTabBtns = document.querySelectorAll('#vips .tab-btn');
  const vipTabContents = document.querySelectorAll('#vips .tab-content');

  if (vipTabBtns.length === 0) {
    console.warn("⚠️ No se encontraron botones de pestañas VIP");
    return;
  }

  vipTabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      const targetTab = btn.dataset.tab;
      
      vipTabBtns.forEach(b => b.classList.remove('active'));
      vipTabContents.forEach(content => content.classList.remove('active'));
      
      btn.classList.add('active');
      const targetContent = document.querySelector(`#vips [data-content="${targetTab}"]`);
      if (targetContent) {
  ...
  // ========== FUNCIÓN GENÉRICA PARA INICIALIZAR CADA VIP ==========
  function initVipSection(prefijo) {
    const vipsInput = document.getElementById(`vips-${prefijo}-input`);
    const vipsPreview = document.getElementById(`vips-${prefijo}-preview`);
    const vipsTabla = document.getElementById(`vips-${prefijo}-tabla`);
    const btnLoad = document.getElementById(`btn-vips-${prefijo}-load`);
    const btnClear = document.getElementById(`btn-vips-${prefijo}-clear`);
    const btnCopiar = document.getElementById(`btn-vips-${prefijo}-copiar`);

    if (!vipsInput || !vipsPreview || !vipsTabla) {
      console.warn(`⚠️ Elementos del módulo VIP ${prefijo} no encontrados`);
      return;
    }
    ...
    // ========== COPIAR CON DISEÑO PROFESIONAL ==========
    btnCopiar.addEventListener("click", async () => {
      if (typeof html2canvas === 'undefined') {
        alert("❌ Error: html2canvas no está disponible. Asegúrate de incluir la librería.");
        return;
      }

      const ticket = vipsPreview.querySelector(".valor-celda")?.textContent || "N/A";
      
      // Crear contenedor temporal con clases CSS
      const tempContainer = document.createElement('div');
      tempContainer.className = 'vip-temp-container';
      
      // Obtener el nombre del VIP
      let tituloVip = '';
      if (prefijo === 'ufinet') tituloVip = 'VIP UFINET';
      else if (prefijo === 'amazon') tituloVip = 'REPORTES VIP AMAZON & AT&T';

      // Crear header profesional
      const header = document.createElement('div');
      header.className = 'vip-temp-header';
      header.innerHTML = `<h2>${tituloVip}</h2>`;

      // Crear tabla mejorada
      const tablaClonada = vipsTabla.cloneNode(true);
      tablaClonada.className = 'vip-temp-table';
      ...
      try {
        ...
        canvas.toBlob(async (blob) => {
          if (navigator.clipboard && window.ClipboardItem) {
            try {
              const textoTicket = `No. de Ticket: ${ticket}`;
              const blobText = new Blob([textoTicket], { type: "text/plain" });
              ...
              alert("✅ Imagen copiada al portapapeles con formato profesional");
            } catch (err) {
              console.error("Error copiando:", err);
              fallbackDownload(canvas, ticket, prefijo);
            }
          } else {
            fallbackDownload(canvas, ticket, prefijo);
          }
    ...
    function fallbackDownload(canvas, ticket, prefijo) {
      const link = document.createElement("a");
      link.href = canvas.toDataURL("image/png");
      link.download = `vip-${prefijo}-${ticket.replace(/[^a-zA-Z0-9]/g, '_')}.png`;
      link.click();
      
      const textoTicket = `No. de Ticket: ${ticket}`;
      navigator.clipboard.writeText(textoTicket);
      alert("✅ Imagen descargada y texto copiado al portapapeles");
    }

    console.log(`✅ Módulo VIP ${prefijo.toUpperCase()} inicializado`);
  }

  // ========== INICIALIZAR LAS 2 SECCIONES VIP ==========
  initVipSection('ufinet');
  initVipSection('amazon');

  console.log("✅ Sistema completo de VIPs inicializado con 2 pestañas");
}
